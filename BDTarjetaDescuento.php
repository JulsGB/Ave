...
/**
 * Description of BDTarjetaDescuento
 *
 * @author Samsung
 */

require_once ("../BDatosConexion.php");
class BDTarjetaDescuento {
     private $BD = null;
    ...
    public function DamePorID($id) {
        ...
        if (!$datos->EOF) {
                $result = new TarjetaDescuento();
                $result->setId($datos->fields["id"]);
                $result->setEmail($datos->fields["descripcion"]);
                $result->setPassword($datos->fields["descuento"]);
                return $result;
        }
        return $result;
    }
